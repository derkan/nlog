...
func NewItemPool(callDepth int, level common.Level, formatters ...common.Formatter) ItemPool {
...
func (p ItemPool) Get(lvl common.Level, prefix string, subDepth int) *Item {
	item := p.p.Get().(*Item)
	item.pool = p
	item.lvl = lvl
	item.prefix = prefix
